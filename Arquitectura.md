...
```
   ...
   ¿configured == false?
    ...
    sí              no
     ...
     ▼              ▼
┌───────────┐  ┌────────────┐
│UNCONFIGURED│  │ CONNECTING │  (conecta STA, sincroniza hora,
│(AP siempre │  │ (sin AP    │   comprueba OTA; LEDs rotando
│  activo)   │  │  todavia)  │   verde/amarillo/rojo; maximo 20s)
└─────┬──────┘  └──────┬─────┘
      │ formulario      │ wifi+hora listas, o timeout
      │ guardado        ▼
      │          ┌───────────────┐
      │          │ PORTAL_WINDOW │  (AP + STA en paralelo; 15s iniciales
      │          │               │   sin nadie conectado, sin limite si
      │          └──────┬────────┘   hay alguien; cierra al desconectar)
      │                 │ timeout sin nadie conectado,
      │                 │ o se desconecta el ultimo cliente
      ▼                 ▼
 [guarda NVS,      ┌──────────┐
...
- **UNCONFIGURED**: `WiFi.softAP` anunciando indefinidamente (modo `WIFI_AP`, sin STA — no hay credenciales aún). No hay fase `CONNECTING` (nada que conectar todavía), pero `portal_.begin()` sigue tardando unos segundos por el escaneo de redes visibles (para el `<select>` de SSID); los LEDs rotan (`LedMode::ROTATE`, arrancado ya en `AppStateMachine::begin()`) hasta que el AP está realmente listo.
- **CONNECTING**: solo quien ya tiene `configured == true`. Arranca la STA (`wifi_.begin()`) pero **todavía no** el AP/portal: intenta conectar a la WiFi guardada, sincronizar la hora por NTP y comprobar actualizaciones OTA (`AppStateMachine::performOtaCheck()`) antes de que nadie pueda ver la página, para que esta se sirva ya con el resultado final en vez de depender de que se auto-refresque mientras el usuario podría estar rellenando el formulario (ver `## OTA`). LEDs rotando (`LedMode::ROTATE`, ver `## LED Controller`) como indicador de "cargando". Pasa a `PORTAL_WINDOW` en cuanto wifi+hora están listas, o a los `kConnectingTimeoutMs` (20s) si no (WiFi inalcanzable, credenciales cambiadas...) — nunca deja el dispositivo sin forma de reconfigurarse.
- **PORTAL_WINDOW**: se entra siempre desde `CONNECTING`, con la STA ya conectada (o todavía intentándolo). Aquí el ESP32 mantiene el AP del portal **y** la WiFi real en paralelo (`WIFI_MODE_APSTA`) — necesario para que la comprobación/actualización OTA del portal (disponible en esta ventana, ver `## OTA`) tenga salida a internet, tanto la que ya se intentó en `CONNECTING` como si la STA conecta más tarde (banderas `otaCheckTriggered_`, ver `## OTA`). Espera 15s a que alguien se conecte al AP; si nadie lo hace, pasa a `RUNNING`. Si alguien se conecta, la ventana se queda abierta sin límite de tiempo mientras siga conectado (`WiFi.softAPgetStationNum() > 0`, consultado cada `loop()`), y se cierra en el momento en que se desconecta — sin depender de ningún timeout adicional. Al entrar en `RUNNING`, `enterRunning()` solo repite `wifi_.begin()` si la STA no quedó ya conectada desde aquí.
...
- `ROTATE` desde el primer `led_.set()` en `AppStateMachine::begin()`, antes incluso de decidir `UNCONFIGURED` o `CONNECTING`: cubre tambien el escaneo de redes de `portal_.begin()` (unos segundos) en el caso `UNCONFIGURED`, que si no se quedaria con los LEDs apagados. En `CONNECTING` (ver `## Máquina de estados`) sigue rotando mientras se intenta conectar a la WiFi/NTP y comprobar OTA antes de abrir el portal. En ambos casos, `updateLedForCurrentState()` lo sustituye por el patron definitivo en cuanto el portal esta listo.
