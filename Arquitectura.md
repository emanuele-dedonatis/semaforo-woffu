...
3. `GET /` sirve un formulario HTML normal (controles nativos: texto, password, `<input type="time">` para las horas — con selector nativo en móvil, `<input type="number">` para los campos numéricos), prellenado con la configuración actual guardada. El campo SSID usa un `<input list>` enlazado a un `<datalist>` con las redes detectadas en el escaneo del paso 1: en navegadores que lo soportan aparece como desplegable nativo, pero el campo sigue siendo de texto libre (necesario para redes ocultas que no aparecen en el escaneo, o si el escaneo no encontró nada). La página muestra también la versión de firmware actual (`FIRMWARE_VERSION`, ver `## OTA — detalle de implementación`), útil para confirmar visualmente que un OTA se aplicó. Por el mismo motivo, también se imprime por Serial al arrancar (`main.cpp`).
...
- **Feedback de un flasheo con éxito sin consola serie**: `httpUpdate.update()` bloquea el `loop()` (no hay `portal_.loop()` mientras dura la descarga+instalación) y, si tiene éxito, reinicia el dispositivo dentro de la propia llamada — no hay forma de que el mensaje "instalando..." llegue nunca a una petición HTTP en curso, y cualquier estado en RAM (`ProvisioningPortal::otaStatusMessage_`) se pierde en el reboot. Para tener constancia en el portal (no solo por Serial) hace falta que sobreviva al reinicio: `OtaUpdater::setBeforeFlashCallback()` se invoca justo antes de `httpUpdate.update()` (único punto de no retorno antes del reboot) y `AppStateMachine` lo usa para llamar a `Config::markOtaPending(from, to)`, que persiste `ota_from`/`ota_to` en NVS (namespace `cfg`, fuera de `DeviceConfig`). Al arrancar, `AppStateMachine::begin()` consume esa nota una sola vez (`Config::takeOtaNote()`) y, comparando `ota_to` con `FIRMWARE_VERSION` actual, decide si mostrar éxito o "se intentó pero arrancó con otra versión" en el portal. Si el flasheo falla sin llegar a reiniciar, `handlePortal()` limpia la nota (`Config::clearOtaNote()`) para que no reaparezca en un reinicio posterior sin relación. Limitación conocida: solo es visible si el cliente sigue conectado (o se reconecta a tiempo) durante la ventana de portal del arranque siguiente; si nadie se conecta en esos 10s, el portal se cierra y el mensaje no llega a verse hasta la próxima vez que se abra (para entonces la nota ya se consumió).
