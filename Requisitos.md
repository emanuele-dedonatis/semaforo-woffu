...
- Polling cada minuto dentro de la **ventana de encendido/apagado** configurada por el usuario (por defecto 07:30–19:00, configurable). Dentro de esa ventana, el propio dispositivo consulta a Woffu la jornada del día (`GET /api/svc/core/users/{userId}/diarysummaries/workday`) una vez al día, solo para saber si hoy es fin de semana o festivo (`isWeekend`/`isHoliday`):
  - **Fuera de la ventana de encendido, fin de semana o festivo**: apagado (tanto el LED como las llamadas a la API de fichaje). Si por algún fallo no se puede consultar la jornada del día, se asume que hoy es laborable (más agresivo pero seguro) hasta el día siguiente.
  - Todas estas decisiones (por qué se apaga, por qué se enciende) se registran de forma explicativa por el puerto serie.
- Sin reintentos ante fallo de conexión (WiFi o API Woffu): un fallo pasa el LED a ámbar y se reintenta en el siguiente ciclo de polling normal (cada minuto), sin backoff adicional. Excepción: si no se consigue conectar a la WiFi configurada, o si Woffu rechaza las credenciales configuradas (usuario/password incorrectos), se registra un error por el puerto serie y el LED pasa a ámbar parpadeando en vez de fijo, para distinguir un problema de configuración persistente de un fallo puntual.
...
- **Fichaje/desfichaje normal**: el lector escucha continuamente mientras el dispositivo está encendido (ver `## Funcionalidad MVP` arriba) — no con el semáforo apagado. Al acercar una tarjeta, los LEDs rotan rápido (indicador de "procesando"):
...
Una vez abierto el portal, aplica tanto si la ventana es de 30s (WiFi conectada) como si es indefinida (sin WiFi):
